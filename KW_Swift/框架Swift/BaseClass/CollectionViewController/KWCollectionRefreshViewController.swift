//
//  KWCollectionRefreshViewController.swift
//  Whatever
//
...
class KWCollectionRefreshViewController: KWCollectionViewController, KWRefreshProtocol {
    ...
    var pageCount: String? = defalutPage {
        didSet {
            if Int(page) ?? 1 >= Int(pageCount ?? "1") ?? 1 {
...
//        if page >= pageCount ?? defalutPage {
        if Int(page) ?? 1 >= Int(pageCount ?? "0") ?? 1 {
            self.collectionView.kw.footer?.endRefreshingWithNoMoreData()
        } else {
            self.collectionView.kw.footer?.resetNoMoreData()
        }
...
extension KWCollectionRefreshViewController {
